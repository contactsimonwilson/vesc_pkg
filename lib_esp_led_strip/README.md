...
| `ext-esp_led-seg-color-fade` | `(i rate)` | ease this segment's colour toward a change instead of switching it, same units as `ext-esp_led-fade` (32nds of the remaining gap per 33 ms, clamped 0..32). 0 is instant and is the default, so a segment behaves as it always did until it opts in. Per segment rather than global because some colours are information rather than decoration - an alarm that fades in reads as a slow alarm. A colour of 0 means "take it from the palette", so a fade only happens between two real colours; with either end at 0 the target lands at once, which also avoids fading up from black when a segment switches from a palette effect to a solid one. Reset by `ext-esp_led-seg-def` |
...
| `ext-esp_led-fade` | `(rate)` | brightness easing, master and per segment: fraction of the remaining gap closed per 33 ms in 32nds (0 = instant, default 15 = ~47%/33 ms). Colour easing is separate and per segment - see `ext-esp_led-seg-color-fade` |
...
Effects: 0 off (all pixels dark, whatever the colour), 1 custom (consumer-supplied pixels via `ext-esp_led-seg-pixel` / `-pixels`), 2 solid, 3 breathe, 4 chase, 5 rainbow, 6 sparkle, 7 comet, 8 gauge (fill by `fx-val`, pulses when `spd` > 0), 9 strobe, 10 larson/knight-rider, 11 felony (halves alternate red/blue), 12 theater (marching marquee), 13 wipe (fill then wipe to black), 14 waves (overlapping slow waves), 15 candle (warm flicker), 16 heartbeat (double pulse), 17 turn signal (splits the strip at its centre; `fx-val` selects the mode - 0 off, then left/right/hazard x solid/blink/sweep as `1 + side*3 + style`; defaults to amber when no colour is set). Each side is `(len + 1) / 2` pixels, so on an odd LED count the centre pixel belongs to both sides - that keeps the two halves the same length, which is what makes hazard sweep symmetric and stay in step.
