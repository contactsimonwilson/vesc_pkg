# Express Native Lib Test

Test package for native library loading on the VESC Express. It builds the same small C library for every supported chip (ESP32-C3, C6, S3 and P4), picks the right binary at runtime with `(sysinfo 'hw-target)` and runs a short self-test that prints to the LispBM console.
...
builds the library for all four chips (needs the `riscv32-esp-elf` and `xtensa-esp32s3-elf` toolchains in the path, the `c_libs/RVfplib` submodule and `vesc_tool` for packaging).
