    ...
    (if (init-humidity) {
        ...
        (loopwhile t {
            ...
            (if has-aht20 {
                ...
                (var hum_raw (+ (shl (bufget-u8 rx-aht20 1) 12)
                                (shl (bufget-u8 rx-aht20 2) 4)
                                (shr (bufget-u8 rx-aht20 3) 4)))
                (var temp_raw (+ (shl (bitwise-and (bufget-u8 rx-aht20 3) 0x0F) 16)
                                 (shl (bufget-u8 rx-aht20 4) 8)
                                 (bufget-u8 rx-aht20 5)))

                (var hum (* (/ hum_raw 1048576.0) 100.0))
                (var hum_temp (- (* (/ temp_raw 1048576.0) 200.0) 50.0))
