...
PARAMS = [
    ...
    # --- LEDs: front strip ----------------------------------------------
    ("led_front_strip_type", "Front Strip", (E, STRIP_TYPES), 0,
     "Front strip mode. PWM Highbeam drives a separate highbeam pin."),
    ("led_front_pin", "Front Pin", (I, -1, 48, ""), 8,
     "GPIO of the front strip data line."),
    ("led_front_num", "Front LED Count", (I, 0, 512, ""), 11,
     "Number of LEDs on the front strip."),
    ...
    ("led_front_highbeam_pin", "Front Highbeam Pin", (I, -1, 48, ""), -1,
     "PWM highbeam GPIO for the front (Strip mode PWM Highbeam)."),

    # --- LEDs: rear strip -----------------------------------------------
    ("led_rear_strip_type", "Rear Strip", (E, STRIP_TYPES), 0,
     "Rear strip mode. PWM Highbeam drives a separate highbeam pin."),
    ("led_rear_pin", "Rear Pin", (I, -1, 48, ""), 9,
     "GPIO of the rear strip data line."),
    ("led_rear_num", "Rear LED Count", (I, 0, 512, ""), 11,
     "Number of LEDs on the rear strip."),
    ...
    ("led_rear_highbeam_pin", "Rear Highbeam Pin", (I, -1, 48, ""), -1,
     "PWM highbeam GPIO for the rear (Strip mode PWM Highbeam)."),

    # --- LEDs: status bar -----------------------------------------------
    ("led_status_strip_type", "Status Strip", (E, STATUS_STRIP_TYPES), 0,
     "Status bar strip mode."),
    ...
    # --- LEDs: footpad --------------------------------------------------
    ("led_footpad_strip_type", "Footpad Strip", (E, STATUS_STRIP_TYPES), 0,
     "Footpad strip mode."),
    ...
    # --- LEDs: button ---------------------------------------------------
    ("led_button_strip_type", "Button LED", (E, STATUS_STRIP_TYPES), 0,
     "Single button LED mode."),
    ...
    ("log_append_gnss", "Log GNSS", (B,), 0,
     "Append GNSS position to log samples."),
...
GROUPS = [
    ("Features", [
        ("General", ["led_enabled", "bms_enabled", "pubmote_enabled",
                     "log_enabled", "humidity_enabled", "can_id",
                     "can_loop_delay"]),
    ]),
    ("LEDs", [
...
def emit_lookup(path):
    ...
    a("")
    a("typedef enum { CFG_F32 = 0, CFG_I32, CFG_U8 } cfg_field_type;")
    a("")
    a("typedef struct {")
    a("    const char *name;")
    a("    uint16_t offset;")
    ...
    a("} cfg_field_t;")
    a("")
    a("static const cfg_field_t cfg_fields[] = {")
    for name, _ln, kind, _d, _desc in iter_params():
        k = kind[0]
        t = {F: "CFG_F32", I: "CFG_I32", E: "CFG_U8", B: "CFG_U8"}[k]
        a('    {"%s", offsetof(FaConfig, %s), %s},' % (name, name, t))
    a("};")
    a("")
    a("#define CFG_FIELD_COUNT (sizeof(cfg_fields) / sizeof(cfg_fields[0]))")
    a("")
    a("// CONF_LOOKUP_H_")
    a("#endif")
    open(path, "w", encoding="utf-8", newline="\n").write("\n".join(out) + "\n")
